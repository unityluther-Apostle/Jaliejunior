...
# ============================================================
# NAVY BLUE + WHITE PRIMARY THEME
# ============================================================
ui.colors(primary='#1e3a8a', secondary='#ffffff', accent='#f59e0b')
    ...
    def load_data():
        ...
        with content_area:
            try:
                ...
                if not raw_rows:
                    with ui.card().classes('w-full p-12 items-center bg-white/90 backdrop-blur-md rounded-3xl shadow-sm border border-blue-100'):
                        ui.icon('folder_open', size='3rem').classes('text-[#1e3a8a]')
                        ui.label('No nursery records found.').classes('text-[#172554] font-semibold mt-2')
                else:
                    nursery_table = (
                        ui.table(columns=columns, rows=raw_rows, row_key='id')
                        .classes('w-full bg-white/95 backdrop-blur-md rounded-3xl shadow-xl border border-blue-100 overflow-hidden')
                        .props('flat bordered wrap-cells')
                    )
                    nursery_table.add_slot('body-cell-actions', """
                        <q-td :props="props">
                            <q-btn icon="edit" flat dense color="primary" class="hover:bg-blue-50 rounded-lg" @click="$parent.$emit('edit_nursery', props.row)"></q-btn>
                        </q-td>
                    """)
...
def edit_nursery_record(record, on_save):
    with ui.dialog() as dialog, ui.card().classes(
        'w-full max-w-[700px] p-6 md:p-8 bg-gradient-to-br from-blue-50'
        ' via-white to-indigo-50 rounded-3xl shadow-2xl border border-blue-100'
    ):
        with ui.row().classes('items-center gap-3 mb-4'):
            with ui.avatar(color='primary', text_color='white').props('size=md'):
                ui.icon('edit_note', size='1.5rem').classes('text-white')
            ui.label(f"Edit Nursery Record: {record.get('pupil_name', '')}").classes(
                'text-xl md:text-2xl font-black text-[#172554]'
            )

        with ui.grid(columns=1).classes('w-full gap-4 md:grid-cols-2'):
            fields = {
                'payment_code': ui.input('Payment Code', value=record.get('payment_code', '')).classes('w-full bg-white/90 rounded-xl'),
                'pupil_name': ui.input('Pupil Name', value=record.get('pupil_name', '')).classes('w-full bg-white/90 rounded-xl'),
                'class_level': ui.input('Class Level', value=record.get('class_level', '')).classes('w-full bg-white/90 rounded-xl'),
                'term': ui.input('Term', value=record.get('term', '')).classes('w-full bg-white/90 rounded-xl'),
                'age': ui.input('Age', value=record.get('age', '')).classes('w-full bg-white/90 rounded-xl'),
                'color': ui.input('Color', value=record.get('color', '')).classes('w-full bg-white/90 rounded-xl'),
                'days_present': ui.input('Days Present', value=record.get('days_present', '')).classes('w-full bg-white/90 rounded-xl'),
                'days_absent': ui.input('Days Absent', value=record.get('days_absent', '')).classes('w-full bg-white/90 rounded-xl'),
                'date': ui.input('Date', value=record.get('date', '')).classes('w-full bg-white/90 rounded-xl'),
                'listening_speaking_sequencing': ui.input('Listening / Speaking & Sequencing', value=record.get('listening_speaking_sequencing', '')).classes('w-full bg-white/90 rounded-xl'),
                'music_rhymes_dance': ui.input('Music / Rhymes & Dance', value=record.get('music_rhymes_dance', '')).classes('w-full bg-white/90 rounded-xl'),
                'punctuality': ui.input('Punctuality', value=record.get('punctuality', '')).classes('w-full bg-white/90 rounded-xl'),
                'numeracy': ui.input('Numeracy', value=record.get('numeracy', '')).classes('w-full bg-white/90 rounded-xl'),
                'vocabulary': ui.input('Vocabulary', value=record.get('vocabulary', '')).classes('w-full bg-white/90 rounded-xl'),
                'literacy': ui.input('Literacy', value=record.get('literacy', '')).classes('w-full bg-white/90 rounded-xl'),
                'general_science': ui.input('General Science', value=record.get('general_science', '')).classes('w-full bg-white/90 rounded-xl'),
                'environmental_awareness': ui.input('Environmental Awareness', value=record.get('environmental_awareness', '')).classes('w-full bg-white/90 rounded-xl'),
                'writing_skills': ui.input('Writing Skills', value=record.get('writing_skills', '')).classes('w-full bg-white/90 rounded-xl'),
                'physical_education': ui.input('Physical Education', value=record.get('physical_education', '')).classes('w-full bg-white/90 rounded-xl'),
                'gods_creation': ui.input("God's Creation", value=record.get('gods_creation', '')).classes('w-full bg-white/90 rounded-xl'),
                'sharing': ui.input('Sharing', value=record.get('sharing', '')).classes('w-full bg-white/90 rounded-xl'),
                'smartness': ui.input('Smartness', value=record.get('smartness', '')).classes('w-full bg-white/90 rounded-xl'),
                'news': ui.input('News', value=record.get('news', '')).classes('w-full bg-white/90 rounded-xl'),
                'stories': ui.input('Stories', value=record.get('stories', '')).classes('w-full bg-white/90 rounded-xl'),
                'life_skills': ui.input('Life Skills', value=record.get('life_skills', '')).classes('w-full bg-white/90 rounded-xl'),
                'general_comment': ui.input('General Comment', value=record.get('general_comment', '')).classes('w-full bg-white/90 rounded-xl'),
                'headteachers_comment': ui.input("Headteacher's Comment", value=record.get('headteachers_comment', '')).classes('w-full bg-white/90 rounded-xl'),
                'next_term_begins_on': ui.input('Next Term Begins On', value=record.get('next_term_begins_on', '')).classes('w-full bg-white/90 rounded-xl'),
                'class_teacher': ui.input('Teacher', value=record.get('class_teacher', '')).classes('w-full bg-white/90 rounded-xl'),
            }
        ...
        def save():
            try:
                ...
                try:
                    ...
                    conn.close()
                ui.notify('✨ Nursery record updated successfully!', color='primary', icon='check_circle')
                dialog.close()
                on_save()
            except Exception as e:
        ...
        ui.button('Save Changes', on_click=save).classes(
            'w-full mt-6 bg-gradient-to-r from-[#1e40af] to-[#1e3a8a] text-white'
            ' font-extrabold py-3.5 rounded-2xl hover:from-[#1e3a8a]'
            ' hover:to-[#172554] transition-all shadow-lg shadow-blue-900/20'
        )
    dialog.open()
    ...
    try:
        ...
        try:
            if level_type == 'Upper Primary':
                ...
                subject_keys = [
                    'literacy_i', 'literacy_ii', 'reading', 'luganda', 'mathematics',
                    'english', 'social_studies', 'science', 're_religious_education'
                ]
...
def edit_record(record, on_save):
    with ui.dialog() as dialog, ui.card().classes(
        ...
        with ui.row().classes('items-center gap-3 mb-4'):
            with ui.avatar(color='primary', text_color='white').props('size=md'):
                ui.icon('edit_note', size='1.5rem').classes('text-white')
            ui.label(f"Edit Record: {record['Name']}").classes('text-xl md:text-2xl font-black text-[#172554]')

        with ui.grid(columns=1).classes('w-full gap-4 md:grid-cols-2'):
            fields = {
                'Name': ui.input('Name', value=record['Name']).classes('w-full bg-white/90 rounded-xl'),
                'Admin': ui.input('Admin', value=record['Admin']).classes('w-full bg-white/90 rounded-xl'),
                'Class': ui.input('Class', value=record['Class']).classes('w-full bg-white/90 rounded-xl'),
                'Maths': ui.number('Maths', value=record['Maths']).classes('w-full bg-white/90 rounded-xl'),
                'English': ui.number('English', value=record['English']).classes('w-full bg-white/90 rounded-xl'),
                'SST': ui.number('SST', value=record['SST']).classes('w-full bg-white/90 rounded-xl'),
                'Science': ui.number('Science', value=record['Science']).classes('w-full bg-white/90 rounded-xl'),
                'Remarks': ui.input('Remarks', value=record['Remarks']).classes('w-full bg-white/90 rounded-xl'),
            }
        ...
        def save():
            ...
            if new_avg >= 80:
                new_grade = 'Division 1'
            elif new_avg >= 60:
                new_grade = 'Division 2'
            elif new_avg >= 50:
                new_grade = 'Division 3'
            elif new_avg >= 40:
                new_grade = 'Division 4'
            else:
                new_grade = 'Division U'
            ...
            try:
                ...
                ui.notify('✨ Record and automatic totals/grades updated successfully!', color='primary', icon='check_circle')
                dialog.close()
                on_save()
            except Exception as e:
                ui.notify(f'Update failed: {e}', type='negative')
...
def edit_lower_record(record, on_save):
    with ui.dialog() as dialog, ui.card().classes(
        ...
        with ui.row().classes('items-center gap-3 mb-4'):
            with ui.avatar(color='primary', text_color='white').props('size=md'):
                ui.icon('edit_note', size='1.5rem').classes('text-white')
            ui.label(f"Edit Lower Primary: {record['pupil_name']}").classes('text-xl md:text-2xl font-black text-[#172554]')

        with ui.grid(columns=1).classes('w-full gap-4 md:grid-cols-2'):
            fields = {
                'payment_code': ui.input('Payment Code', value=record.get('payment_code', '')).classes('w-full bg-white/90 rounded-xl'),
                'pupil_name': ui.input('Pupil Name', value=record['pupil_name']).classes('w-full bg-white/90 rounded-xl'),
                'class_level': ui.input('Class', value=record['class_level']).classes('w-full bg-white/90 rounded-xl'),
                'term': ui.input('Term', value=record['term']).classes('w-full bg-white/90 rounded-xl'),
                'literacy_i': ui.number('Lit I', value=record['literacy_i']).classes('w-full bg-white/90 rounded-xl'),
                'literacy_ii': ui.number('Lit II', value=record['literacy_ii']).classes('w-full bg-white/90 rounded-xl'),
                'reading': ui.number('Reading', value=record['reading']).classes('w-full bg-white/90 rounded-xl'),
                'luganda': ui.number('Luganda', value=record['luganda']).classes('w-full bg-white/90 rounded-xl'),
                'mathematics': ui.number('Maths', value=record['mathematics']).classes('w-full bg-white/90 rounded-xl'),
                'english': ui.number('English', value=record['english']).classes('w-full bg-white/90 rounded-xl'),
                'social_studies': ui.number('S.S.T', value=record['social_studies']).classes('w-full bg-white/90 rounded-xl'),
                'science': ui.number('Science', value=record['science']).classes('w-full bg-white/90 rounded-xl'),
                're_religious_education': ui.number('R.E', value=record['re_religious_education']).classes('w-full bg-white/90 rounded-xl'),
                'class_teacher': ui.input('Teacher', value=record['class_teacher']).classes('w-full bg-white/90 rounded-xl'),
            }
        ...
        def save():
            try:
                ...
                ui.notify('✨ Lower primary record updated successfully!', color='primary', icon='check_circle')
                dialog.close()
                on_save()
            except Exception as e:
                ui.notify(f'Update failed: {e}', type='negative')
        ...
        with content_area:
            try:
                ...
                if not raw_rows:
                    with ui.card().classes('w-full p-12 items-center bg-white/90 backdrop-blur-md rounded-3xl shadow-sm border border-blue-100'):
                        ui.icon('folder_open', size='3rem').classes('text-[#1e3a8a]')
                        ui.label('No lower primary records found.').classes('text-[#172554] font-semibold mt-2')
                else:
                    ...
                    lower_table.add_slot('body-cell-actions', """
                        <q-td :props="props">
                            <q-btn icon="edit" flat dense color="primary" class="hover:bg-blue-50 rounded-lg" @click="$parent.$emit('edit_lower', props.row)"></q-btn>
                        </q-td>
                    """)
        ...
        with content_area:
            try:
                ...
                if not raw_rows:
                    with ui.card().classes('w-full p-12 items-center bg-white/90 backdrop-blur-md rounded-3xl shadow-sm border border-blue-100'):
                        ui.icon('folder_open', size='3rem').classes('text-[#1e3a8a]')
                        ui.label('No upper primary records found.').classes('text-[#172554] font-semibold mt-2')
                else:
                    ...
                    student_table.add_slot('body-cell-actions', """
                        <q-td :props="props">
                            <q-btn icon="edit" flat dense color="primary" class="hover:bg-blue-50 rounded-lg" @click="$parent.$emit('edit', props.row)"></q-btn>
                        </q-td>
                    """)
...
def performance_analytics_hub_content():
    ...
    with content_container:
        with ui.row().classes('items-center gap-3'):
            with ui.avatar(color='primary', text_color='white', size='md').classes('shadow-md shadow-blue-900/20'):
                ui.icon('insights', size='1.5rem').classes('text-white')

            with ui.column().classes('gap-0'):
                ui.label('Per-Class Performance Hub').classes('text-2xl font-black text-[#172554]')
                ui.label('Select a section and class to view isolated performance metrics, top achievers, and intervention lists.').classes('text-xs text-blue-900/70 font-medium')
        ...
        selected_dropdown = ui.select(
            ...
            options=all_options,
            value=all_options[0]
        ).classes('w-full md:w-96 bg-white/95 backdrop-blur-md rounded-2xl shadow-sm border border-blue-100')
        ...
        def update_analytics_view():
            ...
            if not val or 'No Classes Found' in val:
                with metrics_display_area:
                    with ui.card().classes('w-full p-8 items-center bg-white/90 rounded-3xl shadow-sm border border-blue-100'):
                        ui.label('No classes available for analysis.').classes('text-[#172554] font-medium')
                return
            ...
            with metrics_display_area:
                if not data:
                    with ui.card().classes('w-full p-10 items-center bg-white/95 rounded-3xl shadow-sm border border-blue-100'):
                        ui.icon('info', size='2.5rem').classes('text-[#1e3a8a]')
                        ui.label(f'No student records found for {class_name}.').classes('text-[#172554] font-semibold mt-2')
                    return

                with ui.row().classes('w-full gap-4 flex-col md:flex-row'):
                    with ui.card().classes('flex-1 p-6 bg-gradient-to-br from-[#1e3a8a] to-[#172554] text-white rounded-3xl shadow-lg shadow-blue-900/20 relative overflow-hidden'):
                        ui.icon('groups', size='2.5rem').classes('absolute right-4 bottom-4 opacity-15 text-blue-300')
                        ui.label('Total Enrolled').classes('text-xs font-bold text-blue-200 uppercase tracking-wider')
                        ui.label(str(data['total_students'])).classes('text-3xl font-black mt-1')

                    with ui.card().classes('flex-1 p-6 bg-gradient-to-br from-[#2563eb] to-[#1e40af] text-white rounded-3xl shadow-lg shadow-blue-900/20 relative overflow-hidden'):
                        ui.icon('percent', size='2.5rem').classes('absolute right-4 bottom-4 opacity-15 text-blue-200')
                        ui.label('Class Average Score').classes('text-xs font-bold text-blue-100 uppercase tracking-wider')
                        ui.label(f"{data['class_avg']}%").classes('text-3xl font-black mt-1 font-mono')

                    with ui.card().classes('flex-1 p-6 bg-gradient-to-br from-[#0ea5e9] to-[#2563eb] text-white rounded-3xl shadow-lg shadow-blue-800/20 relative overflow-hidden'):
                        ui.icon('trending_up', size='2.5rem').classes('absolute right-4 bottom-4 opacity-15 text-sky-100')
                        ui.label('Best Subject').classes('text-xs font-bold text-sky-100 uppercase tracking-wider')
                ...
                with ui.row().classes('w-full gap-6 mt-2 flex-col md:flex-row'):
                    with ui.card().classes('flex-1 p-6 bg-gradient-to-br from-blue-50 via-indigo-50/40 to-white border border-blue-200/80 rounded-3xl shadow-xl'):
                        with ui.row().classes('items-center gap-3 mb-4'):
                            with ui.avatar(color='primary', text_color='white', size='sm').classes('shadow-sm'):
                                ui.icon('workspace_premium', size='1.2rem').classes('text-white')
                            ui.label(f'Top Achievers — {class_name}').classes('text-md font-black text-[#172554]')
                        ...
                        if not data['top_students']:
                            ...
                            for idx, s in enumerate(data['top_students'], 1):
                                ...
                                with ui.row().classes('w-full justify-between py-2.5 px-3 bg-white/90 rounded-2xl my-1.5 items-center border border-blue-100 shadow-sm'):
                                    with ui.row().classes('items-center gap-3'):
                                        ui.badge(str(idx)).classes(f'{badge_colors} px-2 py-0.5 rounded-full text-xs')
                                        ui.label(name).classes('font-bold text-sm text-[#172554]')

                                    ui.label(f'{round(score, 1)}%').classes('font-mono font-black text-sm text-[#1e40af]')
                    ...
                    with ui.card().classes('flex-1 p-6 bg-gradient-to-br from-amber-50 via-orange-50/50 to-white border border-amber-200/80 rounded-3xl shadow-xl'):
                        with ui.row().classes('items-center gap-3 mb-4'):
                            with ui.avatar(color='amber-7', text_color='white', size='sm').classes('shadow-sm'):
                                ui.icon('warning', size='1.2rem').classes('text-white')
                            ui.label(f'Academic Intervention — {class_name}').classes('text-md font-black text-amber-950')

                        if not data['at_risk']:
                            with ui.row().classes('w-full p-4 bg-white/90 rounded-2xl items-center gap-2 border border-amber-100'):
                                ui.icon('check_circle', size='1.2rem').classes('text-[#1e3a8a]')
                                ui.label('No students require urgent intervention.').classes('text-xs font-semibold text-[#172554]')
                        else:
                            for s in data['at_risk']:
                                name = s.get('Name') or s.get('pupil_name') or 'Unknown'
                                score = s.get('Average') or s.get('_calc_avg') or 0

                                with ui.row().classes('w-full justify-between py-2.5 px-3 bg-white/90 rounded-2xl my-1.5 items-center border border-amber-100 shadow-sm'):
...
def staff_chat_content():
    chat_container = ui.column().classes(
        ...
        ' from-slate-50 to-blue-50/40 rounded-3xl shadow-inner border'
        ' border-blue-100 gap-3'
    )
            ...
            try:
                ...
                if not rows:
                    with ui.column().classes('w-full items-center justify-center my-16 gap-2'):
                        with ui.avatar(color='blue-2', text_color='blue-10', size='lg').classes('shadow-sm'):
                            ui.icon('local_cafe', size='1.8rem').classes('text-blue-10')

                        ui.label('The kettle is boiling, but nobody has spoken yet!').classes('text-[#172554] text-sm font-bold mt-2')
                        ui.label('Drop a complaint about the printer or start the morning gossip.').classes('text-blue-900/70 text-xs italic')
                else:
                    ...
                    for r in rows:
                        ...
                        bubble_bg = (
                            'bg-gradient-to-r from-[#1e40af] to-[#172554] text-white rounded-tr-sm shadow-md shadow-blue-900/10'
                            if is_me
                            else 'bg-white text-slate-800 rounded-tl-sm border border-blue-100 shadow-sm'
                        )

                        with ui.column().classes(f'w-full {align_cls} gap-0.5 my-1.5'):
                            with ui.row().classes('items-center gap-2 px-1'):
                                if not is_me:
                                    ui.label(sender).classes('text-[11px] font-bold text-blue-900')
                                ...
                                if is_me:
                                    ui.button(
                                        ...
                                        'text-stone-400 hover:text-red-600 text-[10px] p-0 min-h-0 min-w-0'
                                    )
                            ...
                            with ui.card().classes(f'{bubble_bg} p-3.5 rounded-2xl max-w-[85%] md:max-w-[75%]'):
                                ...
                                with ui.row().classes('w-full justify-end items-center gap-1 mt-1'):
                                    ui.label(r['timestamp']).classes('text-[9px] text-blue-200' if is_me else 'text-stone-400')

                                    if is_me:
                                        ui.icon('done_all', size='12px').classes('text-blue-200')
            except Exception as e:
                ui.label(f'⚠️ Printer jam in chat: {e}').classes('text-red-500 text-xs')

    with ui.column().classes('w-full max-w-3xl mx-auto bg-white/95 backdrop-blur-md rounded-3xl shadow-xl border border-blue-100 overflow-hidden'):
        with ui.row().classes('items-center justify-between w-full bg-gradient-to-r from-[#1e3a8a] via-[#172554] to-[#0f172a] px-5 py-4 text-white border-b border-blue-900'):
            with ui.row().classes('items-center gap-3'):
                with ui.avatar(size='md').classes('bg-white/20 text-white font-extrabold shadow-sm backdrop-blur-sm'):
                    ui.icon('forum', size='1.2rem').classes('text-white')

                with ui.column().classes('gap-0'):
                    ui.label('Staff Common Room').classes('text-base font-black text-white')
                    ui.label(current_vibe).classes('text-[11px] text-blue-200 font-medium')

            with ui.row().classes('items-center gap-1'):
                ui.badge('Live Sync', color='amber-7').props('dense').classes('text-[10px] text-white font-bold')
                ui.timer(3.0, load_messages)
                ui.button(icon='refresh', on_click=load_messages).props('flat round dense').classes('text-white hover:text-blue-200')

        load_messages()

        quick_emojis = ['😂', '💀', '🔥', '👀', '☕', '📉', '💯', '🙏', '⚠️', '🤷‍♂️', '🤦‍♀️', '🍎', '📝', '📢']

        with ui.row().classes('w-full gap-1 items-center px-4 py-2 bg-blue-50/60 border-t border-blue-100 overflow-x-auto'):
            ui.label('Vibe Check:').classes('text-[10px] text-blue-900 font-black uppercase mr-1 shrink-0')
            ...
            for emo in quick_emojis:
                ...
                ui.button(emo, on_click=insert_emoji).props('flat dense').classes(
                    'text-sm bg-white border border-blue-200 rounded-xl px-2 py-0.5 text-stone-700 hover:bg-blue-100 shrink-0 shadow-sm'
                )

        with ui.row().classes('w-full items-center gap-2 p-3.5 bg-white border-t border-blue-100'):
            msg_input = (
                ui.input(placeholder='Say something before the bell rings...')
                .classes('flex-1 bg-slate-100 text-slate-900 rounded-2xl px-4 border-none')
            ...
            ui.button(icon='send', on_click=send_message).classes(
                'bg-gradient-to-r from-[#1e40af] to-[#1e3a8a] text-white rounded-2xl'
                ' p-2 h-10 w-10 shadow-md shadow-blue-900/20'
                ' hover:from-[#1e3a8a] hover:to-[#172554] transition-all'
            )
...
def teacher():
    ...
    ui.query('.nicegui-content').classes('w-full min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50/60 p-4 md:p-8')
    ...
    with ui.column().classes('w-full items-center'):
        with ui.column().classes('w-[98%] md:w-[95%] max-w-7xl gap-6 md:gap-8'):
            with ui.card().classes(
                'w-full p-8 md:p-12 bg-gradient-to-r from-[#1e3a8a] via-[#172554]'
                ' to-[#0f172a] shadow-xl rounded-3xl text-center border border-blue-500/20'
                ' relative overflow-hidden'
            ):
                ui.icon('school', size='6rem').classes('absolute -right-8 -bottom-8 opacity-10 text-blue-300')
                ui.icon('auto_stories', size='6rem').classes('absolute -left-8 -top-8 opacity-10 text-sky-300')

                ui.label("Teacher's Dialogue").classes('text-3xl md:text-5xl font-black text-white tracking-tight')
                ui.label('School Report & Academic Management System').classes('text-blue-200 text-sm md:text-base font-medium mt-1')

            with ui.card().classes(
                'w-full p-6 bg-gradient-to-r from-blue-100/80 via-blue-50 to-white'
                ' border-l-8 border-[#1e40af] rounded-3xl shadow-sm border border-blue-100'
            ):
                with ui.row().classes('items-center gap-2 mb-1'):
                    ui.icon('menu_book', size='1.2rem').classes('text-[#1e40af]')
                    ui.label('📖 Daily Inspiration').classes('text-[#172554] font-extrabold text-xs tracking-wider uppercase')

                ui.label(random.choice(verses)).classes('text-[#172554] italic text-lg md:text-xl font-medium')

            with ui.tabs().classes(
                'w-full bg-white/95 backdrop-blur-md p-2 rounded-3xl shadow-lg'
                ' shadow-blue-900/5 border border-blue-100 overflow-x-auto flex-nowrap'
            ) as tabs:
                tabs.classes('items-center justify-start md:justify-center')

                ui.tab('Dashboard', icon='dashboard').classes('rounded-2xl transition-all duration-300 hover:bg-blue-50 text-[#172554] font-bold')
                ui.tab('Class Analytics', icon='analytics').classes('rounded-2xl transition-all duration-300 hover:bg-blue-50 text-[#172554] font-bold')
                ui.tab('Upper Primary entry', icon='add_circle').classes('rounded-2xl transition-all duration-300 hover:bg-blue-50 text-[#172554] font-bold')
                ui.tab('Upper Primary Records', icon='assignment').classes('rounded-2xl transition-all duration-300 hover:bg-blue-50 text-[#172554] font-bold')
                ui.tab('Lower Primary entry', icon='add_circle').classes('rounded-2xl transition-all duration-300 hover:bg-blue-50 text-[#172554] font-bold')
                ui.tab('lower Primary Records', icon='assignment').classes('rounded-2xl transition-all duration-300 hover:bg-blue-50 text-[#172554] font-bold')
                ui.tab('Nursery entry', icon='child_care').classes('rounded-2xl transition-all duration-300 hover:bg-blue-50 text-[#172554] font-bold')

                nursery_records_tab = ui.tab('Nursery Records', icon='list_alt').classes('rounded-2xl transition-all duration-300 hover:bg-blue-50 text-[#172554] font-bold')

                ui.tab('Staff Chat', icon='forum').classes('rounded-2xl transition-all duration-300 hover:bg-blue-50 text-[#172554] font-bold')
                ui.tab('Logout', icon='logout').classes('rounded-2xl transition-all duration-300 hover:bg-red-50 text-red-600 font-bold')
            ...
            with ui.tab_panels(tabs, value='Dashboard').classes('w-full bg-transparent'):
                with ui.tab_panel('Dashboard'):
                    ...
                    with ui.row().classes('w-full gap-4 mb-6 flex-col md:flex-row'):
                        with ui.card().classes('flex-1 p-6 bg-gradient-to-br from-[#1e3a8a] to-[#172554] text-white shadow-xl shadow-blue-900/15 rounded-3xl relative overflow-hidden'):
                            ui.icon('calendar_month', size='2.5rem').classes('absolute right-4 bottom-4 opacity-15 text-blue-300')
                            ui.label('ACADEMIC CALENDAR').classes('text-xs font-bold text-blue-200 uppercase tracking-wider')
                            ui.label(f'{current_term}, {current_year}').classes('text-3xl font-black mt-1')
                            ui.label('Primary Section Active').classes('text-xs text-blue-100/80 mt-1 font-medium')
                        ...
                        with ui.card().classes('flex-1 p-6 bg-gradient-to-br from-[#2563eb] to-[#1e3a8a] text-white shadow-xl shadow-blue-900/15 rounded-3xl relative overflow-hidden'):
                            ui.icon('badge', size='2.5rem').classes('absolute right-4 bottom-4 opacity-15 text-blue-200')
                            ui.label('TOTAL ENROLLED').classes('text-xs font-bold text-blue-100 uppercase tracking-wider')
                            ui.label(str(total_pupils)).classes('text-3xl font-black mt-1')
                            ui.label(f'Upper: {upper_count} | Lower: {lower_count} | Nursery: {nursery_count}').classes('text-xs text-blue-100/80 mt-1 font-medium')
                        ...
                        with ui.card().classes('flex-1 p-6 bg-gradient-to-br from-[#0ea5e9] to-[#1e40af] text-white shadow-xl shadow-blue-800/15 rounded-3xl relative overflow-hidden'):
                            ui.icon('military_tech', size='2.5rem').classes('absolute right-4 bottom-4 opacity-15 text-sky-200')
                            ui.label('P.7 CANDIDATE CLASS').classes('text-xs font-bold text-sky-100 uppercase tracking-wider')
                            ui.label(str(p7_count)).classes('text-3xl font-black mt-1')
                            ui.label('Registered for PLE Assessment').classes('text-xs text-sky-100/80 mt-1 font-medium')

                    metrics = get_dashboard_metrics()

                    with ui.row().classes('items-center mb-6 gap-3'):
                        with ui.avatar(color='primary', text_color='white', size='md').classes('shadow-md'):
                            ui.icon('monitoring', size='1.5rem').classes('text-white')

                        ui.label('Performance Analytics').classes('text-2xl font-black text-[#172554]')

                    if metrics:
                        with ui.row().classes('w-full gap-6 flex-col md:flex-row'):
                            with ui.card().classes('w-full md:w-1/3 p-6 shadow-xl bg-white/95 backdrop-blur-md rounded-3xl border border-blue-100'):
                                with ui.row().classes('items-center gap-2 mb-4'):
                                    ui.icon('trending_up', size='1.4rem').classes('text-[#2563eb]')
                                    ui.label('Subject Performance Trends').classes('text-md font-black text-[#172554]')

                                with ui.row().classes('w-full justify-around mt-2'):
                                    with ui.column().classes('items-center p-4 bg-blue-50 rounded-2xl flex-1 mr-2 border border-blue-100'):
                                        ui.icon('arrow_upward', size='1.8rem').classes('text-[#1e40af]')
                                        ui.label('Top').classes('text-xs text-stone-400 font-bold uppercase mt-1')
                                        ui.label(metrics['best']).classes('text-lg font-black text-[#1e40af]')

                                    with ui.column().classes('items-center p-4 bg-amber-50 rounded-2xl flex-1 ml-2 border border-amber-100'):
                                        ui.icon('arrow_downward', size='1.8rem').classes('text-amber-600')
                            ...
                            with ui.card().classes('w-full md:w-[63%] p-6 shadow-xl bg-white/95 backdrop-blur-md rounded-3xl border border-blue-100'):
                                with ui.row().classes('w-full gap-8 flex-col md:flex-row'):
                                    with ui.column().classes('flex-1 bg-amber-50/50 p-4 rounded-2xl border border-amber-100'):
                                        with ui.row().classes('items-center gap-2 mb-3'):
                                            with ui.avatar(color='amber-7', text_color='white', size='xs'):
                                                ui.icon('warning', size='1rem').classes('text-white')
                                    ...
                                    with ui.column().classes('flex-1 bg-blue-50/70 p-4 rounded-2xl border border-blue-200'):
                                        with ui.row().classes('items-center gap-2 mb-3'):
                                            with ui.avatar(color='primary', text_color='white', size='xs'):
                                                ui.icon('workspace_premium', size='1rem').classes('text-white')

                                            ui.label('Rising Stars').classes('text-sm font-black text-[#172554]')
                                        ...
                                        if not metrics['rising']:
                                            ...
                                            for s in metrics['rising']:
                                                with ui.row().classes('w-full justify-between py-1.5 px-2 bg-white rounded-2xl my-1 border border-blue-100 shadow-sm items-center'):
                                                    ui.label(f"{s['Name']} ({s['Class']})").classes('text-xs font-bold text-stone-800')
                                                    ui.badge(f"{round(s['Average'] or 0, 1)}%").classes('bg-blue-100 text-blue-900 font-mono font-bold')
                    else:
                        with ui.card().classes('w-full p-12 items-center bg-white/95 rounded-3xl shadow-xl border border-blue-100'):
                            ui.icon('analytics', size='3rem').classes('text-[#1e3a8a]')
                            ui.label('No academic data available for analysis.').classes('text-[#172554] font-bold mt-2')

                with ui.tab_panel('Class Analytics'):
                    with ui.card().classes('w-full p-6 bg-white/95 backdrop-blur-md rounded-3xl shadow-xl border border-blue-100'):
                ...
                with ui.tab_panel('Logout'):
                    with ui.card().classes('w-full max-w-[320px] mx-auto items-center p-8 bg-white/95 backdrop-blur-md rounded-3xl shadow-xl border border-blue-100'):
                        with ui.avatar().classes('bg-red-100 text-red-600 shadow-sm').props('size=xl'):
                            ui.icon('logout', size='2.5rem').classes('text-red-600')
